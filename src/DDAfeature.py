...
def print_eic_ms(mzML_file):
    ...
    for element in tree.iter(tag="{http://psi.hupo.org/ms/mzml}spectrum"):
        if element.findtext(".//*{http://psi.hupo.org/ms/mzml}binary"):
            ...
            if mslevel == "1":
                ...
                ms2_scans.append(
                    (
                        element.find(".//*[@accession='MS:1000744']").get("value"),
                        store_scan(element),
                    )
                )
            else:
    ...
    def mz_slice(ms1_scans, sorted_ms2_pre_mz):
        ...
        mz_min, mz_max = sorted_ms2_pre_mz[0] - 0.02, sorted_ms2_pre_mz[-1] + 0.03
        ...
        for (pos, mz0), (pos1, mz1) in zip(slice_cut, slice_cut[3:]):
            if (
                pos + min_group_size < pos1
            ):  # and max(I for _,_,I in dp_sub)>min_highest_I:
                dp_sub = data_points[pos:pos1]  # .tolist()
                ms2pos0 = bisect_left(sorted_ms2_pre_mz, mz0 + 0.008999)
                ms2pos1 = bisect_left(sorted_ms2_pre_mz, mz1 - 0.008999)
                if ms2pos0 < ms2pos1:
    ...
    with open("ms1feature_" + basename0 + ".txt", "w") as writepeak:
        ...
        writepeak.write("MS1\n")
        for peak in peak_list:
            pos0 = bisect_left(sorted_ms2_pre_mz, peak.mz - 0.01)
            pos1 = bisect_left(sorted_ms2_pre_mz, peak.mz + 0.01)

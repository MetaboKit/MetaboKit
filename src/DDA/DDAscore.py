...
#libpaths.append('LipidCreatorValidStudy_MRM_Workklist_V1_forHyungWon.txt')
...
cpd_list={
        ...
        '':0,
        #'MSDIAL-InsilicoMSMS-Lipids-neg'
...
def read_lib(libpath):
    ...
    if 'LipidCreatorValidStudy_MRM_Workklist_V1_forHyungWon.txt' in libpath0:
        ...
        for nnn,line in enumerate(qq):
            ...
            #if not charge.isdigit():print(lsp[11])
            ...
            frag_mz=[lsp[3]]
            frag_I=['1']
            #print(ms1mz,adduct,charge,name,frag_mz,frag_I)
            #sys.exit()
...
def get_cpds():#from libs
    ...
    for ent,name in lib_dict.items():
        ...
        #thres=bpfilter[0]*max(frag_I)
        #I_mz_list=[(i,mz) for i,mz in zip(frag_I,frag_mz) if i>thres]
        #I_mz_list=sorted(I_mz_list,reverse=True)[:10]
        ...
        Mmass=ms1mz*charge
        adduct=adduct.split(',')
        #if 671.4469<float(ms1mz)<671.447:
        #    print(ms1mz,charge,Mmass,name)
        ...
        #adductset.add((adduct,ms1mz,Mmass))

############################
        #if len(frag_mz)>1:
        lib_ent.append(Ent(Mmass,name,frag_mz,frag_I,None,None,RT))
    #print(len(lib_ent),len(lib_dict))
        ...
        for nn,line in enumerate(ms_sc):
            ...
            if line.rstrip():
                ...
                if lsp[0]=='scan':
                    ...
                    #I_count=collections.Counter(I_list)
                    #repeated=[x for x,y in sorted(I_count.items()) if y>9]# and x<.1*max(I_list)]
                    #thres_I=max(repeated) if repeated else 0
                    #mz_I_dict={x:y for x,y in zip(mz_list,I_list) if y>thres_I}# and abs(x-ms1mz)>3.3}
                    #I_mz_list=sorted(((y,x) for x,y in mz_I_dict.items()),reverse=True)[:10]

                    #thres=bpfilter[0]*max(I_list)
                    #I_mz_list=[(i,mz) for i,mz in zip(I_list,mz_list) if i>thres]
                    #I_mz_list=sorted(I_mz_list,reverse=True)[:10]
                    ...
                    if mz_i_list:
                        ...
                        #if max(i_list)>2*min(i_list):
                        #dp_scan.append(Spec(ms1mz,rt,[x for x,_ in mz_i_list],i_list,mz_list,I_list))
                        dp_scan.append(Spec(ms1mz,rt,[x for x,_ in mz_i_list],i_list,[x for _,x in I_mz_list],[x for x,_ in I_mz_list]))
    #print('thres',max(thres))
...
def print_score(mzML_file):
    ...
    if bg is not None:
        ...
        for ii,ms2sc in enumerate(ms2scans):
            ...
            if pos1!=pos0: 
                #print([x[:2] for x in ms2scans_bg[pos0:pos1]])
                #print(ms2sc[:2])
    ...
    def readms1peak():
        ...
        for ii in range(len(ms1peaks)):
            ...
            ##doubly
            #pos0=bisect_left(ms1peaks,(peak0.mz-iso_diff/2-err_bd,))
            #pos1=bisect_left(ms1peaks,(peak0.mz-iso_diff/2+err_bd,))
            #if pos0!=pos1:
            #    peak1=min(ms1peaks[pos0:pos1],key=lambda p: abs(peak0.rt-p.rt))
            #    #if current mass is not monoisotopic mass, replace with mono mass
            #    if abs(peak0.rt-peak1.rt)<2 and peak0.auc<peak1.auc:
            #        ms1peaks[ii]=Peak(*peak0[:-1],peak1.mmz)
            #        if peak1.mmz==peak1.mz:
            #            pos0=bisect_left(ms1peaks,(peak1.mz+iso_diff-err_bd,))
            #            pos1=bisect_left(ms1peaks,(peak1.mz+iso_diff+err_bd,))
            #            if pos0!=pos1:
            #                peak_=min(ms1peaks[pos0:pos1],key=lambda p: abs(peak1.rt-p.rt))
            #                if abs(peak1.rt-peak_.rt)<2 and peak_.auc<peak0.auc<peak1.auc:
            #                    doub+=1
            #                    peak_double.add(peak1)
            ...
            if pos0!=pos1:
                ...
                if abs(peak0.rt-peak1.rt)<1 and peak0.auc<peak1.auc:
                    ...
                    if peak1.mmz==peak1.mz:
                        single+=1
        #print('single,double',single,doub)
    ...
    #set ms2 spectrum to monoisotopic precursor peak
    def ms2_spectrum():
        #peak_double=set()
        #peak_single=set()
        ...
        for nn,spec in enumerate(ms2scans):
            ...
            #if s_peak is not None and s_peak.mz==s_peak.mmz: #append monoisotopic peak only #only use ms2 with feature 
                ms2_wp.append((spec,s_peak))
            #elif .3<s_peak.mz-s_peak.mmz<.7:
            #    #peak_double.add((s_peak[:2],ms1peaks[bisect_left(ms1peaks,(s_peak.mmz,))][:2]))
            #    peak_double.add((s_peak.mmz,s_peak.mz,s_peak.rt,s_peak.mz-s_peak.mmz))
            #    #peak_double.add((s_peak.mz))
            #else:
            #    peak_single.add((s_peak.mmz,s_peak.mz,s_peak.rt,s_peak.mz-s_peak.mmz))
        #peak_single-=peak_double
        #print(len(peak_double))
        #print(len(peak_single))
        #for x in sorted(peak_double):print([round(y,3) for y in x])
        #print()
        #for x in sorted(peak_single):print([round(y,3) for y in x])
        #sys.exit()
        #if ms2_wp==sorted(ms2_wp): print('adsfa')

        # keep one spectrum(close to apex) per peak
        peak_ms2=dict()
        ms2_wp_=[]
        for s,p in ms2_wp:
        ...
        #peak_ms2=sorted(peak_ms2.items())
        #print(peak_ms2[bisect_left(peak_ms2,((sorted(peak_double)[0],),))])

        #find isf-parent pair
        ...
        for ii in range(len(ms2_wp)-1,-1,-1):
            ...
            if peak:# and peak.mz==peak.mmz:
                for jj in range(bisect_left(ms2_wp,((ms2.ms1mz,),))):
                    ...
                    if i_p and abs(peak.rt-i_p.rt)<ISF_rt_diff and pos0!=pos1 and max(ms2.I)*.1<max(ms2.I[pos0:pos1]): #require ISF to be n% of base peak
                    #if i_p and abs(peak.rt-i_p.rt)<ISF_rt_diff and pos0!=pos1:
                        ms2_I=[]
                        i_ms2_I=[]
                        xfrag=set()
                        #ms2.I=[x for x,y in zip(ms2.I_all,ms2.mz_all) if y<i_ms2.ms1mz][:10]
                        #ms2.mz=[y for y in ms2.mz_all if y<i_ms2.ms1mz][:10]
                        #for f_i,f_mz in ((x,y) for x,y in zip(i_ms2.I,i_ms2.mz) if y<i_ms2.ms1mz+.01):
                        ...
                        if sum(1 for x in ms2_I if x>0)>1:
                            ...
                            #cs=cos_sim(i_ms2_I,ms2_I)
                            if cs>ISF_score:
                                isf_sc[jj].append((cs,ii))

        #isf_sc=[[x for x in ic if x[0]>max(ic)[0]-.1] for ic in isf_sc ]
                ...
                for ent in lib_ent_:
                    ...
                    for nn,(f_mz,f_I) in enumerate((x,y) for x,y in zip(ent.mz,ent.I) if (charge0*premz-x)>3.3):
                        ...
                        if pos0!=pos1:
                            ...
                            ms2_I.append(0)
                    #if 666.617<premz<666.62 and 710<peak.rt<720 and adduct0=='M+NH4':
                    #    print(ent,sum(1 for x in ms2_I if x>0),spec.mz)
                    if sum(1 for x in ms2_I if x>0)>=min_peaks: #min number of matching peak
                    #if ms2_I:# and .8<sum(ms2_I)/sum(spec.I):
                        ###add umatched experimental peaks
                        ...
                        #cs=sum(1 for x,y in zip(ent_I,ms2_I) if (x>0 and y>0))/len(ms2_I)
                        #if 127.045<premz<127.055 and 120<spec.rt<140: print('cs',cs)
                        if cs>MS2_score:
                            score_ent.append((cs,ent,adduct0))
            if score_ent:
            #if score_ent and not(peak and peak.mz!=peak.mmz):#this will cut possible matches if ISF
    ...
    def print_ann(ann_,adduct,peak,name):
        ...
        #for i,mz in sorted(zip(adduct[2].I_all,adduct[2].mz_all),reverse=True):
        ...
        for adduct_match,spec,peak in map(mass_matching, range(len(ms2_wp))):
            for adduct in adduct_match:
                if len(adduct[1])==3: # if lipidblast
                    ...
                    pos0,pos1=adduct[1]
                    #nameset={x[0]+' '+(x[2] if x[2] else x[3]) for _,x in dlist[pos0:pos1]}
                    nameset={x for _,x in dlist[pos0:pos1]}
                    #if not nameset:
                    #    nameset=['unknown_'+str(uk_count)]
                    #    uk_count+=1
                    name0='\n'.join(sorted(nameset))
                    #name0='\n'.join(' '.join(x[1][:1]+x[1][3:4]) for x in dlist[pos0:pos1])
                #if name0:
                id_quant_ma[name0,adduct[0]].append((adduct,peak))
            if not adduct_match and "NoMatch" not in lib_types[0]:
                #print_ann(una_,('?',(0,0),spec),peak,'unknown')
                ...
                #filter by topNfrag
                #I_mz_list=sorted(I_mz_list,reverse=True)[:bpfilter[1]]
    ...
    def print_all(id_quant):
        ...
        for (name,adductid),adducts in id_quant.items():
            for adduct,peak in adducts:
                #if peak and 127.04<peak.mz<127.06 and 243.09<peak.mmz<243.1: print('dfd')
                #if name or (peak and peak.mz!=peak.mmz):
...
list(map(print_score, mzML_files))
    ...
    #            name=line.split('=')[1]+' gnps'
    #            adduct=line.split('=')[1].rsplit(None,1)[1]
    #            adduct=adduct.strip('[]')
    ...
    #            adduct=tuple(re.split('(\-|\+)',adduct))
    #            #print(adduct)
    #            if frag_mz and charge!=0 and adduct[0]=='M' and all((x in cpd_list) for x in adduct[2::2]):
    #                lib_dict[ms1mz+' '+charge+' '+','.join(frag_mz)+' '+','.join(frag_I)+' '+','.join(adduct)+' NA'].append(name)

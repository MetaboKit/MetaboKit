...
Ann=collections.namedtuple('Ann',('nn dotp premz rt mz_l I_l auc feat formu m_peaks'))
...
for nn,ann_file in enumerate(ann_files):
    ...
    for line in it_ann:
        if line.startswith("NAME:"):
            ...
            all_dat[name_id].append(Ann(nn,dotp,premz,rt,mz_list,I_list,auc,feat,formu,m_peaks))
...
for key,vv in isf_dict.items():
    if len(vv)>1:
        pmz_rt_mz=[tuple(float(x) for x in re.findall("\d+\.\d+",v[0])[:3]) for v,_ in vv]
...
for key,names in sorted(mz_rt_name.items()):
    if any(x[0][0].startswith('ISF of ') for x in names) and all(not eligible_parent(x)for x in names):
...
with open('ann_All.txt','w') as cpd_ann, \
        open('quant_All.txt','w') as quant_auc, \
        open('All.txt','w') as cpd_una:
    quant_auc.write('group\tISF\tname\tadduct\tformula\tfeature_m/z\tmatching_peaks(median)\tMin.\t1st Qu.\tMedian\t3rd Qu.\tMax.\t%detected\t')
    ...
    for n0,names in sorted([inner for outer in [list(x.items()) for x in mz_rt_n_dict.values()] for inner in outer]+mz_rt_n):
        ...
        for name in names:
            ...
            for d in all_dat0[name]:
                if name[0][0].startswith('ISF of ') or (d.nn,d.premz,d.rt) not in isf_dat:
        ...
        if not dat0 or (sum(x.feat for xx in dat0.values() for x in xx)==0 and len(dat0)<len(mzML_files)*.8):
            continue
        for nn,dotp_l in dat0.items():
            sdotp=sorted(dotp_l,key=operator.attrgetter('dotp'),reverse=True)
            dat.append(sdotp[0])
            for p in sdotp[1:]:
                if sdotp[0].dotp-p.dotp<.1:
                    dat.append(p)
                else:
                    break
        dat.sort()
        ...
        prev_n0=n0
        quant_auc.write('\t{}\t{}\t{:.5f}\t{:.1f}'.format(','.join(adductset),','.join(sorted(set(d.formu for d in dat))),statistics.median(premz_l),statistics.median(d.m_peaks for d in dat)))
        ...
        quant_auc.write('\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}\t{:.2f}'.format(s_rt[0]/60,Qu1,Qu2,Qu3,s_rt[-1]/60,len({x.nn for x in dat})/len(mzML_files)))
        ...
        for nn in range(len(mzML_files)):
            ...
            line_str.append(( \
                    ','.join(format(dat_n.auc,'.1f') if dat_n.auc else 'no_ms1_feature' for dat_n in dat[pos0:pos1]), \
    ...
    for pos0,(pmz,rt,ii) in enumerate(s_mz_rt):
        if (pmz,rt,ii) not in i_mz_rt:
            mz_rt_l=[]
            pos1=bisect_left(s_mz_rt,(pmz+.01,),lo=pos0)
    ...
    for grp in grp_mz_rt:
        ...
        if lib_ent_:
            ...
            cpd_una.write('NAME: (putative) {}\n'.format(' --- '.join(list({x.name:None for x in s_lib_ent_}))))
        else:

    ...
    if "hmdb" in libpath and "All.txt" not in libpath:
        ...
        for _, elem in ET.iterparse(open(libpath, "rb")):
            if elem.tag == "{http://www.hmdb.ca}metabolite":
                ...
                elem.clear()
        for file_x in glob.glob(
            str(pathlib.Path.home()) + "/AMD_lib/hmdb_experimental_msms_spectra/*"
        ):
            frag_mz = []
            frag_I = []
            ...
            if hmdb_dat is not None and frag_mz:
                lib_dict[
                    str(float(hmdb_dat[2]) + cpd_list["H"])
                    + " 1 "
                    + ",".join(frag_mz)
                    + " "
                    + ",".join(frag_I)
...
def print_score(mzML_file):
    ...
    cpd_quant.write(
        "compound\tQuantification Mode\tadduct\tmass\tRT(library)\trt\tdot_prod\tp_f_cor\tquant\n"
    )
    ...
    def scoring(cpd):
        ...
        ms1peaks_match = [
            x
            for x in ms1peaks[pos0:pos1]
            if not isinstance(RT, float) or abs(RT - x.rt) < rt_diff
        ]
        if (
            ms2_auc_no_feat and not ms1peaks_match and isinstance(RT, float)
        ):  # if no feature found and RT in library
        ...
        for ms1peak in ms1peaks_match + pseudo_feat:
            ...
            if pos0 == pos1:  # ms1peak in one window only
                ...
                pos = (
                    pos0
                    if ms1peak.mz - startpt[pos0 - 1] > end__pt[pos1] - ms1peak.mz
                    else pos1
                )
            ...
            p_area = [
                x
                for x in ms1scans[
                    bisect_left(ms1scans, (ms1peak.mz - 0.01,)) : bisect_left(
                        ms1scans, (ms1peak.mz + 0.01,)
                    )
                ]
                if rt_l < x.rt < rt_r
            ]
            ...
            for nn, (f_mz_l, f_mz_r, f_I) in enumerate(
                zip(frag_mz_l, frag_mz_r, frag_I)
            ):
                f_area = [
                    x
                    for x in iso[
                        bisect_left(iso, (f_mz_l,)) : bisect_left(iso, (f_mz_r,))
                    ]
                    if rt_l < x.rt < rt_r
                ]
                if f_area:
                    ...
                    if f_area_:
                        ...
                        for pt in f_area:
                            if pt.rt not in f_dict or f_dict[pt.rt] < pt.I:
                                f_dict[pt.rt] = pt.I
                        ms2_maxI = [f_dict.get(rt, 0.0) for rt in ms1rt]
                        pfc[nn] = cos_sim(p_maxI, ms2_maxI)
                        ms2_auc[nn] = (
                            sum(
                                (I0 + I1) * (rt1 - rt0)
                                for rt0, rt1, I0, I1 in zip(
                                    ms1rt, ms1rt[1:], ms2_maxI, ms2_maxI[1:]
                                )
                            )
                            / 2
                        )
        ...
        if score_peaks:
            ...
            for x in score_peaks[1:]:
                if max_score_peaks[0][0] - x[0] < 0.1:
                    max_score_peaks.append(x)
                else:
                    break
            max_peak = max(
                max_score_peaks, key=lambda x: x[2].auc
            )  # pick top scoring ms2 for each entry, if score difference is insignificant use auc
    ...
    def print_mp(mp):
        ...
        if True:
            ...
            for f_mz, f_ann, ms2_pfc, ms2_auc in zip(
                frag_mz, frag_ann, max_peak[1], max_peak[3]
            ):
                if ms2_pfc > -0.5:
                    cpd_quant.write(
                        "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(

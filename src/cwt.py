...
def findridge(eic):
    ...
    I_cut = I_sub[int(min(len(I_sub) * 0.1, len(I_sub) - 1))]
    eic_rt = set()
        ...
        for xx, wave_scale in enumerate(wave_scales):
            ...
            for i, rt0 in enumerate(rt_):
                if rt0 in eic_dict:
                    ...
                    int_I[i] = (
                        (eic_dict[rt0][1] - min_) * math.exp(-tsig2 / 2) * (1.0 - tsig2)
                    )
            coefs[xx][yy] = (
                sum(
                    (I0 + I1) * (rt1 - rt0)
    ...
    for xx, wave_scale in enumerate(wave_scales):
        ...
        while any(y > 0 for _, y in coef_xx):
            ...
            if (max_i != 0 and max_i != len(coef_xx) - 1) and (
                coef_xx[max_i - 1][1] == 0 or coef_xx[max_i + 1][1] == 0
            ):
    ...
    for xx, scale_coef in enumerate(max_map):
        for yy, coef in ((y, c) for y, c in enumerate(scale_coef) if c > 0):
            ...
            for rl in ridgelines:
                if (
                    abs(
                        bisect_left(rt_all, rl[-1].rt) - bisect_left(rt_all, eic_rt[yy])
                    )
                    < 2
    ...
    for rd in ridgelines:
        ...
        auc = (
            sum(
                (eic_dict.get(rt0, (0, 0))[1] + eic_dict.get(rt1, (0, 0))[1])
                * (rt1 - rt0)
        ...
        if 0 < auc and rd.index(peak_loc) < len(rd) - 5:
            ...
            peakmz = sum(eic_dict[rt][0] * eic_dict[rt][1] for rt in rt_sub) / sum(
                eic_dict[rt][1] for rt in rt_sub
            )
